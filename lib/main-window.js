...
module.exports = function (config) {
  ...
  const defaultViews = computed(includeParticipating, (includeParticipating) => {
    const result = [
      '/chat','/public', '/private', '/mentions', '/status'
    ]
  ...
  const container = h(`MainWindow -${process.platform}`, {
    ...
    when(latestUpdate,
      h('div.info', [
        h('a.message -update', { href: 'https://github.com/everlifeai/everlife-community-node/releases/latest' }, [
          h('strong', ['Everlife Explorer ', latestUpdate, i18n(' has been released.')]), i18n(' Click here to download and view more info!'),
