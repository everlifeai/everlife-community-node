...
exports.create = function (api) {
  ...
  return nest('message.html.compose', function ({ shrink = true, isPrivate, participants, meta, hooks, prepublish, placeholder = 'Write a message', draftKey }, cb) {
    ...
    if (draftKey) {
      const draft = window.localStorage[`everlife.drafts.${draftKey}`]
      if (draft) {
        textArea.value = draft
      }
      const draftCW = window.localStorage[`everlife.drafts.contentWarning.${draftKey}`]
      if (draftCW) {
    ...
    function save () {
      ...
      if (draftKey) {
        if (!textArea.value) {
          delete window.localStorage[`everlife.drafts.${draftKey}`]
        } else {
          window.localStorage[`everlife.drafts.${draftKey}`] = textArea.value
        }
        if (!contentWarningInput.value) {
          delete window.localStorage[`everlife.drafts.contentWarning.${draftKey}`]
        } else {
          window.localStorage[`everlife.drafts.contentWarning.${draftKey}`] = contentWarningInput.value
        }
      }
    }
